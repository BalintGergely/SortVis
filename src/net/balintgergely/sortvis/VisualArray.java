...
public final class VisualArray{
	...
	public boolean contains(boolean s,int value){
		int[] a = array(s);
	...
	public int localMinimum(boolean s){
		int[] a = array(s);
		int min1 = a[offset];
	...
	public int localMaximum(boolean s){
		int[] a = array(s);
		int max1 = a[offset];
		for(int i = 1;i < size;i++){
			int z = a[offset+i];
	...
	public int spread(boolean s){
		int[] a = array(s);
		int min1 = a[offset],max1 = min1;
		for(int i = 1;i < size;i++){
			int z = a[offset+i];
			if(z > max1) {
				max1 = z;
	...
	/**
	 * @return the value that is supposed to be at the specified index when the array is sorted.
	 */
	...
	public int compareAndSwap(boolean as,int a,boolean bs,int b){
		...
		fireEvent(x,y,false);
		if(i > 0){
			cd = System.nanoTime()+visualCooldown;
			setCooldown(as,a,cd);
			setCooldown(bs,b,cd);
			aa[a] = y;
			bb[b] = x;
	...
	/**
	 * Copies a sequence of colors from the source index to the target index in the main array. Does not fire an event.
	 * @param sp The source index to copy
	 * @param tp The target index to copy to
	 * @param len The length of the sequence.
	 */
	public void copyColors(int sp,int tp,int len){
		checkInterval(sp,len);
		checkInterval(tp,len);
		System.arraycopy(mainColor, offset+sp, mainColor, offset+tp, len);
	}
	/**
	 * Copies a sequence of colors from the source index in the source array to the target index in the target array. Does not fire an event.
	 * @param source The source array to copy from
	 * @param sp The source index to copy
	 * @param target The target array to copy to
	 * @param tp The target index to copy to
	 * @param len The length of the sequence.
	 */
	public void copyColors(boolean source,int sp,boolean target,int tp,int len){
		checkInterval(sp,len);
		checkInterval(tp,len);
		System.arraycopy(source ? shanColor : mainColor, offset+sp, target ? shanColor : mainColor, offset+tp, len);
	}
	/**
	...
	/**
	 * @return True if the elements in this VisualArray are in ascending order.
	 */
	public boolean localCheck(boolean doEvent){
		int high = offset+size;
		int val = mainArray[offset];
		for(int i = offset+1;i < high;i++){
			int vil = mainArray[i];
			if(doEvent){
				mainCooldown[i] = mainCooldown[i-1] = System.nanoTime()+visualCooldown;
				fireEvent(val,vil,false);
			}
			if(vil < val){
				return false;
			}
