...
public class ImageHandler extends DataBuffer implements IntUnaryOperator {
	...
	public int applyAsInt(int operand) {
		...
		case MODE_COLUMNS:
			...
			break sw;
		case MODE_PIXEL_ROWS:
			while(operand > 0 && offsets[operand-1] == offsets[operand]){
				operand--;
			}
			break sw;
		case MODE_PIXEL_COLUMNS:
			while(operand > 0 && offsets[(((operand-1)%height)*width)+((operand-1)/height)] == offsets[((operand%height)*width)+(operand/height)]){
				operand--;
			}
			break sw;
		}
