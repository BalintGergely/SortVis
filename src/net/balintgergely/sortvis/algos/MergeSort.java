...
public class MergeSort extends ConfigurableSorter{
	...
	public CompletionStage<?> sort(VisualArray vis,Sorter srt, Executor exe) {
		if(lowLevelFirst){
			...
			if(exe == null){
				...
				while(i >= 0){
					int zi = i;
					cml = cml.thenCompose((Object obj) -> sort(vis,this,exe,zi));
					i--;
				}
				return cml;
			}
	...
	public static void mergeOutOfPlace(VisualArray vis,int left,int right,boolean op){
		int len = vis.size;
		int x = 0,y = 0,z = 0,k = 0;
	...
	public static void mergeInPlace(VisualArray vis,int middle,int right){
		int left = 0;
		while(true){
