...
public class SortVisualizer extends JPanel implements DisplayInterface{
	...
	private SortVisualizer() {
		...
		}
		{
			...
			String emptyUnit = "EMPTY";
			settingPanel.add(new JPanel(), emptyUnit);
				...
				selectedSorter.addTreeSelectionListener((TreeSelectionEvent e) -> {
					...
					if(pt != null){
						...
						if(unit != null){
							...
							crd.show(settingPanel, emptyUnit);
						}
					}
				});
			}
	...
	public static Color colorOf(VisualArray data,boolean shan,int i,boolean cooldown,String method){
		switch(method){
		case CONSTANT:
	...
	public static float heightOf(VisualArray data,boolean shan,int i,String method){
		switch(method){
	...
	public static void paintComponentPillars(VisualArray data,Graphics2D gr,int width,int height,long time,String cm,String hm){
		width -= 16;
		height -= 8;
		if(width < data.size){
			width = data.size;
	...
	public static void paintComponentBricks(VisualArray data,Graphics2D gr,int width,int height,long time,String cm,String hm){
		width -= 16;
		height -= 8;
		if(width < data.size){
			width = data.size;
	...
	public static void paintComponentDots(VisualArray data,Graphics2D gr,int width,int height,long time,String cm,String hm){
		width -= 16;
		height -= 8;
		if(width < data.size){
			width = data.size;
	...
	public static void paintComponentRays(VisualArray data,Graphics2D gr,int w,int h,long time,String cm,String hm){
		w -= 8;
		h -= 8;
	...
	public static void paintComponentRayDots(VisualArray data,Graphics2D gr,int w,int h,long time,String cm,String hm){
		w -= 8;
		h -= 8;
		int cx = w/2+4,cy = h/2+4;
		w /= 2;
	...
	public static void paintComponentRayLines(VisualArray data,Graphics2D gr,int w,int h,long time,String cm,String hm){
		w -= 8;
		h -= 8;
		int cx = w/2+4,cy = h/2+4;
		w /= 2;
